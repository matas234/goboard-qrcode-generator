...
def create_go_board(
    ...
    if background == "color":
        board_image = Image.new(
            "RGB", (board_pixel_size, board_pixel_size), color=bg_color
        )
        ...
        board_image = Image.new(
            "RGBA", (board_pixel_size, board_pixel_size), (0, 0, 0, 0)
        )
...
def generate_qr_matrix(data):
    qr = qrcode.QRCode(
        box_size=1, border=0, error_correction=qrcode.constants.ERROR_CORRECT_L
    )  ## change this for error correcting (currently on the lowest)
    qr.add_data(data)
    qr.make(fit=True)
...
def create_go_board_with_qr(
    ...
    black_stone = (
        Image.open(f"assets/b16.png").resize((stone_size, stone_size)).convert("RGBA")
    )
    white_stone = (
        Image.open(f"assets/w16.png").resize((stone_size, stone_size)).convert("RGBA")
    )
...
create_go_board_with_qr(
    ...
    stone_scaling=0.9,
    border_size=100,
    background="transparent", # "color", "texture", or "transparent"
    bg_color=(0,0,0,0)
)
